...
try {
    ...
    // Include debug hint (prod-safe: only messages we created)
    if (!empty($GLOBALS['CLOUDINARY_LAST_ERROR'])) {
        $resp['debug_cloudinary'] = $GLOBALS['CLOUDINARY_LAST_ERROR'];
        $resp['debug_cloud'] = getCloudName();
    }
    if (!empty($GLOBALS['UPLOAD_LAST_ERROR'])) {
        $resp['debug_upload'] = $GLOBALS['UPLOAD_LAST_ERROR'];
        $resp['post_max_size'] = ini_get('post_max_size');
        $resp['upload_max_filesize'] = ini_get('upload_max_filesize');
    }
    }
